			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						var scenarioBonusNums 	 = scenario.split('|')[1].split(',');
						var scenarioGames 		 = scenario.split('|').slice(2);
						...
						for (var bonusNumIndex = 0; bonusNumIndex < bonusNumMultis.length; bonusNumIndex++)
						{
							r.push('<td style="padding-right:50px">');

							canvasIdStr = 'cvsBonusNumTitle' + bonusNumIndex.toString(); 
							elementStr  = 'eleBonusNumTitle' + bonusNumIndex.toString();
							textStr1    = getTranslationByName("titleBonusNum", translations) + bonusNumMultis[bonusNumIndex].toString();
						...
						for (var bonusNumIndex = 0; bonusNumIndex < scenarioBonusNums.length; bonusNumIndex++)
						{
							r.push('<td style="padding-right:50px">');

							canvasIdStr  = 'cvsBonusNumData' + bonusNumIndex.toString(); 
							elementStr   = 'eleBonusNumData' + bonusNumIndex.toString();
							boxColourStr = (scenarioBonusNums[bonusNumIndex][0] == '*') ? colourLime : colourWhite;
							textStr1     = scenarioBonusNums[bonusNumIndex].replace(new RegExp('[*]', 'g'), '');

							showBox(canvasIdStr, elementStr, boxColourStr, colourBlack, textStr1, '');
						...
						r.push('<div style="float:left; margin-top:40px; margin-right:50px">');
						r.push('<table border="0" cellpadding="2" cellspacing="1" class="gameDetailsTable">');
						r.push('<tr class="tableheader">');
						...
						r.push('<div style="float:left; margin-top:40px">');
						r.push('<table border="0" cellpadding="2" cellspacing="1" class="gameDetailsTable">');

						for (gameRow = 0; gameRow < 7; gameRow++)
						{
